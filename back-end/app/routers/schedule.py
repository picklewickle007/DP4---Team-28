...
from app.models import Schedule, History, Patient_profiles # databases
...
def complete_task(task_id: int, session: Session = Depends(get_session)):
    ...
    completed_task = History(patient_id=task.patient_id, psw_id=task.psw_id, task=task.task, start_time = task.start_time, duration=task.duration, date=task.date)
